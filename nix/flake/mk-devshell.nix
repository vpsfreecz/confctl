...
pkgs.mkShell {
  ...
  shellHook = ''
    ...
    mkdir -p "$GEM_ROOT" "$GEM_HOME" "$PWD/.bin" "$PWD/.man/man8"
    ...
    "$bundle_bin" exec md2man-roff "$CONFCTL_SRC/man/man8/confctl.8.md" > "$PWD/.man/man8/confctl.8"
    "$bundle_bin" exec md2man-roff "$CONFCTL_SRC/man/man8/confctl-options.nix.8.md" > "$PWD/.man/man8/confctl-options.nix.8"
