...
rec {
  ...
  getClusterMachines =
    cluster:
    let
      machineAttrs = mapAttrs (name: metaConfig: makeMachine { inherit name metaConfig; }) cluster;
    in
  ...
  getAllAddressesOf =
    ...
    let
      ...
      addresses = flatten (
        map (machine: map (addr: machine // addr) machine.metaConfig.addresses.${"v${toString v}"}) machines
      );
    in
    addresses;
      ...
      { config, ... }:
      {
        options = {
          ...
          string = mkOption {
            ...
            default = null;
            apply = v: if isNull v then "${config.address}/${toString config.prefix}" else v;
